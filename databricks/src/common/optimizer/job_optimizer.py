"""
================================================================================
ETL JOB OPTIMIZER — DATABRICKS VERSION (Full)
================================================================================
This is the FULL Databricks equivalent of glue_job_optimizer.py.
Same analysis depth (volume, skew, quality, scaling, 4-scenario recommendations).

Differences from AWS Glue version:
    - spark.table() instead of create_dynamic_frame.from_catalog()
    - Delta data skipping instead of push_down_predicate
    - Databricks instances instead of G.1X/G.2X/G.4X/G.8X
    - Cost in DBUs ($0.40/DBU Jobs Compute)
    - No GlueContext/DynamicFrame dependency
    - Unity Catalog APIs for metadata

Usage (Databricks notebook):
    result = analyze_and_optimize_databricks_job(
        sources=[
            {"database": "silver_db", "table": "sales", "primary_key": "txn_id",
             "partition_column": "data_dt", "role": "primary"},
            {"database": "silver_db", "table": "dim_product", "primary_key": "prod_id",
             "role": "dimension", "operation": "JOIN", "join_keys": ["prod_id"]},
        ],
        target={"database": "gold_db", "table": "sales_mart"},
        run_mode="overwrite",
        analysis_end_month="202606",
        spark=spark,
        analysis_months_count=3
    )
    import json
...
import sys
# No Glue imports needed for Databricks
# Databricks: use dbutils.widgets or job parameters
from pyspark.context import SparkContext
# Databricks: no GlueContext needed
# Databricks: no Job object needed
...
# Databricks: SparkSession already available as `spark`
# glueContext not needed — use spark.table() directly
# spark already available in Databricks notebooks
# No Glue Job object in Databricks
...
class ETLOptimizationAnalyzer:
    ...
    def __init__(
        ...
        spark: SparkSession,
        # Databricks: no glue_context needed
        ...
        self.spark = spark
        # Databricks: use self.spark directly (no GlueContext)
        
        # AWS clients
        # Databricks: use spark.catalog or Unity Catalog APIs
    ...
    def _auto_detect_target_granularity(self) -> str:
        ...
        try:
            response = self.spark.catalog.getTable(f'{database}.{table}')  # Unity Catalog
            ...
            # Databricks: direct table read (Delta data skipping optimizes automatically)
    ...
    def _calculate_scaling_factor(self, source_analysis: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Databricks: query distinct partition values directly (fast on Delta — uses stats)
            partition_result = self.spark.sql(f"SELECT DISTINCT {source_analysis['metadata']['partition_column']} FROM {database}.{table}").collect()
            
            all_partition_values = [str(row[0]) for row in partition_result]
    ...
    def _detect_partition_type(self, database: str, table: str, etl_partition_column: str,
        ...
        try:
            ...
            # Databricks: direct table read (Delta data skipping optimizes automatically)
            df = self.spark.table(f"{database}.{table}").limit(10)
    ...
    def _estimate_row_size(self, database: str, table: str, num_columns: int) -> int:
        ...
        Uses filter to read only 1 partition (Delta data skipping) for sampling
        ...
        try:
            ...
            if filter_clause:
                # [Databricks] Read one partition via spark.table() + filter.
                # Delta data skipping prunes files automatically based on the filter.
                df = self.spark.table(f"{database}.{table}").filter(filter_clause).limit(1000)
            else:
                # Fallback: direct table read (Delta data skipping still optimizes).
                df = self.spark.table(f"{database}.{table}").limit(1000)
    ...
    def _analyze_volume(self, database: str, table: str, primary_key: str, 
        ...
        # Databricks: use spark.table() — Delta data skipping handles partition pruning
        if filter_clause:
            df = self.spark.table(f"{database}.{table}").filter(filter_clause)
        else:
            df = self.spark.table(f"{database}.{table}")
    ...
    def _generate_recommendations(self) -> List[Dict]:
        ...
        recommendations.append({
            ...
            'action': 'Use .filter() on spark.table() — Delta data skipping prunes files automatically',
...
def analyze_and_optimize_databricks_job(
    sources: List[Dict[str, Any]],
    target: Dict[str, str],
    run_mode: str,
    analysis_end_month: str,                   # ← Required params first
    spark: SparkSession,
    # No GlueContext for Databricks
    analysis_months_count: int = 3,
    output_format: str = "dict"
) -> Dict[str, Any]:
    """
    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║  ETL JOB OPTIMIZER (DATABRICKS) — Analyze tables & recommend cluster config║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    Databricks version of the optimizer. Same analysis as AWS Glue version but:
      • Uses spark.table() + .filter() (Delta data skipping, no DynamicFrame)
      • Recommends Databricks instances (i3, r5, m5) instead of G.1X/G.2X
      • Includes Photon speedup factor in throughput estimates
      • Cost in DBUs instead of DPU-hours
      • No GlueContext dependency
    ...
    ├─────────────────────┼──────────┼─────────────────────────────────────────────────────────────┤
    │ sources             │ ✅ YES   │ List of source table dicts (see PER-SOURCE below)            │
    │ target              │ ✅ YES   │ {"database": "gold_db", "table": "gold_table"}               │
    │                     │          │ Only for naming — optimizer does NOT read this table          │
    │ run_mode            │ ✅ YES   │ "overwrite" = full rewrite | "append" = incremental          │
    │ analysis_end_month  │ ✅ YES   │ "YYYYMM" — end of analysis window (e.g. "202606")           │
    │ spark               │ ✅ YES   │ Active SparkSession (already available in Databricks)        │
    │ analysis_months_count│ ❌ NO   │ Default: 3. Months to sample (1-3). More = better accuracy   │
    │ output_format       │ ❌ NO   │ Default: "dict". Options: "dict" | "json"                    │
    └─────────────────────┴──────────┴─────────────────────────────────────────────────────────────┘

    NOTE: No glue_context needed. Databricks uses spark.table() directly.
          Delta data skipping handles partition pruning automatically.

    ═══════════════════════════════════════════════════════════════════════════════
    PER-SOURCE PARAMETERS (each dict in `sources`)
    ═══════════════════════════════════════════════════════════════════════════════

    ┌─────────────────────┬──────────┬─────────────────────────────────────────────────────────────┐
    │ Key                 │ Required │ Description                                                  │
    ├─────────────────────┼──────────┼─────────────────────────────────────────────────────────────┤
    │ database            │ ✅ YES   │ Unity Catalog: "catalog.schema" or just "schema"             │
    │                     │          │ e.g. "main.silver" or "silver_db"                             │
    │ table               │ ✅ YES   │ Table name, e.g. "sales"                                     │
    │ primary_key         │ ✅ YES   │ Key column for skew analysis (your GROUP BY / join key)      │
    │                     │          │ e.g. "site_code", "customer_id"                               │
    ├─────────────────────┼──────────┼─────────────────────────────────────────────────────────────┤
    │ partition_column    │ ❌ NO    │ Override partition column (default: auto-detect from Delta)   │
    │ role                │ ❌ NO    │ "primary" (default) | "dimension" (gets cache/broadcast rec) │
    │ operation           │ ❌ NO    │ "JOIN" | "UNION" | "JOIN_UNION_GROUPS"                        │
    │ join_keys           │ ⚠️ COND  │ Required if operation="JOIN". e.g. ["product_id"]            │
    │ join_type           │ ❌ NO    │ "INNER" | "LEFT" (default) | "RIGHT"                         │
    │ union_group         │ ❌ NO    │ Group name for UNION size aggregation                        │
    ...
    # SIMPLE (Databricks notebook cell):
    result = analyze_and_optimize_databricks_job(
        sources=[{"database": "main.silver", "table": "sales", "primary_key": "order_id"}],
        target={"database": "main.gold", "table": "sales_mart"},
        run_mode="overwrite",
        analysis_end_month="202606",
        spark=spark
    )
    ...
    result = analyze_and_optimize_databricks_job(
        sources=[
            {"database": "main.silver", "table": "sales", "primary_key": "site_code",
             "partition_column": "data_dt", "role": "primary"},
            {"database": "main.silver", "table": "dim_site", "primary_key": "site_id",
             "role": "dimension", "operation": "JOIN", "join_keys": ["site_code"]},
        ],
        target={"database": "main.gold", "table": "gold_sales"},
        run_mode="overwrite", analysis_end_month="202606", spark=spark
    )
    ...
    Same structure as AWS Glue version:
    {
        "resource_recommendation": {
            "initial_load": {worker_type, num_workers, shuffle_partitions, cost, time},
            "incremental_daily": {...},
            "incremental_weekly": {...},
            "incremental_monthly": {...},
        },
        "spark_configurations": {per-scenario Spark config dicts},
        "caching_recommendations": [...],
        "warnings": [...],
    }

    Instance types recommended: i3.xlarge, i3.2xlarge, i3.4xlarge, r5.2xlarge, etc.
    Cost in DBUs × $0.40 (Jobs Compute pricing).
    Photon factor applied to throughput estimates when Photon runtime detected.
    """
    ...
    try:
        ...
        analyzer = ETLOptimizationAnalyzer(
            ...
            spark=spark,
            # glue_context not used in Databricks
        )
